...
import { Wallet, Shield, CheckCircle, XCircle, Loader2, ExternalLink, ArrowRightCircle } from "lucide-react"
...
export default function WalletConnect({ onAuthSuccess }: WalletConnectProps) {
  ...
  const PAYMENT_RECEIVER = process.env.NEXT_PUBLIC_BASE_PAYMENT_RECEIVER || "0x03173186D626031276F7bfa1f3af0E19f54DdCE6"
  const PAYMENT_AMOUNT = process.env.NEXT_PUBLIC_PAYMENT_AMOUNT_USDC || "0.30"
  ...
  const payAndSearch = async () => {
    ...
    try {
      ...
      const targetChainId = BigInt(BASE_CHAIN_ID)
      ...
      const decimals: number = Number(await usdc.decimals().catch(() => 6))
      const amount = parseUnitsSafe(PAYMENT_AMOUNT, decimals)

      const tx = await usdc.transfer(PAYMENT_RECEIVER, amount)
      toast({ title: "Payment sent", description: `Tx: ${tx.hash.slice(0, 10)}...` })
      ...
      const verifyResp = await fetch("/api/payments/x402/verify", {
        method: "POST",
      ...
      toast({ title: "Request completed", description: "Data retrieved successfully" })
    } catch (err: any) {
      console.error("Payment error:", err)
      setError(err?.message || "Payment or verification failed")
      toast({
  ...
  return (
    <Card className="w-full max-w-md bg-card/90 border-primary/30 backdrop-blur-sm cyber-glow">
      ...
      <CardContent className="space-y-4">
        ...
        {walletAddress && (
          <div className="space-y-3 border border-primary/20 rounded p-4 bg-background/30">
            <div className="space-y-2">
              <label className="text-sm text-muted-foreground">Request</label>
              <textarea
                className="w-full rounded border border-primary/20 bg-background/30 p-2 text-sm text-foreground"
                rows={4}
                value={requestText}
                onChange={(e) => setRequestText(e.target.value)}
                placeholder="Enter query (email, name, etc.)"
              />
            </div>
            ...
            >
              {isPaying ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  Processing payment & search...
                </>
              ) : (
                <>
                  <ArrowRightCircle className="mr-2 h-4 w-4" />
                  Pay 0.30 USDC (Base) & Search
                </>
              )}
            </Button>

            {searchResult && (
              <div className="border border-primary/20 rounded p-3 bg-background/40 text-sm space-y-2 max-h-96 overflow-auto">
